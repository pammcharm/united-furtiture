...
def _product_dict(product):
    data = {
        ...
        "color": product.color,
        "size": product.size,
        "custom_order_available": product.custom_order_available,
        "stock": product.stock,
...
def site_context():
    ...
    return {
        ...
        "filter_options": {
            ...
            "materials": sorted({product["material"] for product in products if product.get("material")}),
            "colors": sorted({product["color"] for product in products if product.get("color")}),
            "sizes": sorted({product["size"] for product in products if product.get("size")}),
            "types": sorted({product["furniture_type"] for product in products if product.get("furniture_type")}),
            "tags": sorted({tag.strip() for product in products for tag in product.get("tags", "").split(",") if tag.strip()}),
        },
